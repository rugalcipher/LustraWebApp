...
export class ApiError extends Error {
  ...
  constructor(init: {
    ...
    super(init.title || SAFE_MESSAGES[init.kind]);
    this.name = "ApiError";
  ...
  /** A message safe to show to end users — never raw exception text. */
  get userMessage(): string {
    return SAFE_MESSAGES[this.kind];
  }
...
export function toFormErrors(e: unknown): Record<string, string> {
  ...
  for (const fe of e.fieldErrors) {
    out[fe.field] = fe.messages[0] ?? "Invalid value";
  }
  return out;
}
